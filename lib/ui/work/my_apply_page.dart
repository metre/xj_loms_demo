...
import 'package:xj_loms_manager/net/apply_list_net.dart';
...
class MyApplyState extends State<MyApplyPage>{


  List<ApplyModel> _list = [];
  ...
  void _post(){
    ApplyListNet().post(context, _lastId, (data) {
      setState(() {
        if(isLoadingMore) {
          _list.addAll(data);
        }else{
  ...
  Widget _itemView(BuildContext context, int index) {
    ApplyModel bean = _list[index];

    Column listView = Column(
        crossAxisAlignment:CrossAxisAlignment.start,
        children: <Widget>[

     Align(alignment: Alignment.centerRight,child: getStateByType(bean.typeX)),
      Row(
    ...
    return InkWell(child: listView, onTap: () {
      Routes.startApplyDetailPage(context, bean.bizId,bean.id,bean.flowId,bean.flowName);
    });
  }
  ...
  Widget getStateByType(String type) {
    String url = "";
    if (type == 'wait') {
      url = 'icon_flow_status_approving.png';
    } else if (type == 'finish') {
